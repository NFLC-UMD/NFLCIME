NFLCIME.dispatchEvent( { type:'ModuleAdd', module:
{
	...
	determineChanges:function(edit, key) {
		...
		var evt = { type:'CursorGetContext', target:edit, textAhead:'', textBehind:'', behindCount:16, aheadCount:16 };
		NFLCIME.dispatchEvent(evt);
		var cxt = { ahead:evt.textAhead, behind:evt.textBehind };
	...
	initialize:function(env, subclassing) {
		...
		for(var i = 0; i < this.componentUnicodeGroups.length; i++) {
			...
			this.componentPatternsLookingForward.push( { type:group.type, regexp:new RegExp('^' + prefix + group.pattern + suffix) } );
			this.componentPatternsLookingBackward.push( { type:group.type, regexp:new RegExp(prefix + group.pattern + suffix + '$') } );
		}
		...
		for(var i = 0; i < this.decomposition.length; i++) {
			...
			this.equivalentSyllables.push( { composed:composed, decomposed:decomposed } );
		}
		// sort the list so the pair with more components come first
		this.equivalentSyllables.sort( function(a, b) { return (b.decomposed.componentCount > a.decomposed.componentCount) ? 1 : -1 } )
	...
	logicalOrder:{
		...
		consonant:3, vowelIndependent:3,
	...
	visualOrder:{
		...
		consonantHalfForms:2,
		consonant:3, vowelIndependent:3,
		consonantAppearingAsSignPre:4, consonantAppearingAsSignPost:4, consonantModifiers:4, vowelSign:4, syllableModifiers:4, vowelModifiers:4, vowelAppearingOnRight:4
	},
	...
	componentUnicodeGroups:[
		// describes how Unicode range assigned to a script is mapped to the different character classes
	],
	decomposition:[
		// describes how some characters in the Unicode range decompose into simpler characters
	]
}
} );