	...
	tinymce.create('tinymce.plugins.IcePlugin', {
		...
		init: function(ed, url) {
			...
			ed.onPostRender.add(function(mgr) {
				...
				ed.serializer.addRules(self.insertTag + '[id|class|title|lang|dir|style|'+self.changeIdAttribute + '|' + self.userIdAttribute + '|' + self.userNameAttribute + '|' + self.timeAttribute + ']');
				ed.serializer.addRules(self.deleteTag + '[id|class|title|lang|dir|style|'+self.changeIdAttribute + '|' + self.userIdAttribute + '|' + self.userNameAttribute + '|' + self.timeAttribute + ']');
			...
			ed.addCommand('initializeice', function(editor) {
				...
				tinymce.DOM.win.setTimeout(function() {
					...
					ed.onLanguageSelected && ed.onLanguageSelected.add(function(e) {
							changeEditor._preventCharInsert = (!!e.language);
					});
					
					ed.onLanguageChange && ed.onLanguageChange.add(function(language) {
							changeEditor._preventCharInsert = (language.lang == '');
					});
