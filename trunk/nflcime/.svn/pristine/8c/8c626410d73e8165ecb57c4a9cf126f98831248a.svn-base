NFLCIME.dispatchEvent( { type:'ModuleAdd', module:
{
	...
	onLanguageChange:function(evt){
		...
		if(editor_lang !=  keyboard_lang && editor.onLanguageChange){
			...
			editor.onLanguageChange.dispatch({lang: keyboard_lang, dir: direction});
		};
	},
	onLanguageSelected:function(evt){
		var editor= evt.editor, lang = evt.language;
		editor.language = lang;
		editor.getElement().setAttribute('lang',(lang || ""));
		if(!lang){
			NFLCIME.dispatchEvent({type:'CursorInsertEmptyText', target : editor.getDoc()});
		};	
		editor.focus();
		editor.getBody().focus();
		NFLCIME.dispatchEvent( { type:'LanguageChange', languageTag:lang});
	},
	//--- Private functions
	attachTinyMCEHandlers:function(tinymce) {
		var editors = tinymce.editors, self=this;
