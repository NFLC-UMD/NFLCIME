...
/*
*
* Declare the NFLCIME object if it was not already present from
* loading the nflcime_f.js script
*
*/
...
/*
*
* Base function for extending NFLCIME class
*
...
/*
*
* Basic class system methods:
*
* 
* addEventListener(eventName,listener,capture):  Add a listener for a specific event (listner['on' + eventName] will be invoked)
* removeEventListener(eventName, listener, capture): Remove an event listener
* dispatchEvent(evt) : Dispatch an event to all listening objects
* onWindowListen(evt) : Attach events to window, prevent textboxes from losing focus to elements marked as "unselectable" in firefox and webkit browsers on mousedown event
* onWindowIgnore(evt) : Detach events from window
* onWindowGetList(evt) : Gets list of windows (from this.windowsHandled property), also cleans up from any windows that were closed
* onServiceApplicable(evt) : Determines whether a given IME service is active for a given element, service names are passed in as a space separated string as evt.service
*   presumes an element attribute of NFLCIME="some list of services", e.g. <textarea NFLCIME="kb cv" /> or <textarea NFCLIME="all" />
* onModuleAdd()
* onModuleRegister()
* onModuleLoad()
* onModuleGetList()
...
NFLCIME.extend(NFLCIME, {
	...
	mapUrlToPackage: function(url) {
		var package = url.split('.');

		if (url != 'kb.js' && url != 'ui.iframe.js' && url != 'cvt.js' && url != 'ui.lang.js' && url != 'ui.kb.js') {
			if (package[0] == 'cvt' || package[0] == 'kb' || package[0] == 'ui' || package[0] == 'editor') {
...
function trace() {};
NFLCIME.initialize();;NFLCIME.dispatchEvent( { type:'ModuleAdd', module:
{
	id:'pers.cookie',
	type:'persistence',
	//--- Event handlers
...
NFLCIME.dispatchEvent( { type:'ModuleAdd', module:
{
	...
	module: {
				...
				case 'safari':
					if (edit.tagName == 'TEXTAREA' || edit.tagName == 'INPUT') {
						if (this.browser == 'firefox') {
							...
							var value = edit.value;
							var beginning = value
								.substr(0, edit.selectionStart) + text;
						...
						var top = caret.offsetTop,
							bottom = top + caret.offsetHeight;
						var left = caret.offsetLeft,
							right = left + caret.offsetWidth;
						for (view = caret.offsetParent; view && view.nodeType == 1; view = view.parentNode) {
							var view_top = view.scrollTop,
								view_bottom = view_top + view.clientHeight;
							var view_left = view.scrollLeft,
								view_right = view_left + view.clientWidth;
							if (view == doc.body) {
		...
		onCursorInsertTag: function(evt) {
			...
			var tag = evt.html;
			var doc = edit.ownerDocument;
			if (edit.tagName == 'TEXTAREA' || edit.tagName == 'INPUT') {} else {
				switch (this.browser) {
					case 'ie':
						// TODO: check to see if the range is in the edit
						var range = doc.selection.createRange();
		...
		onCursorClearSelection: function(evt) {
			...
			switch (this.browser) {
				...
				case 'safari':
					if (edit.tagName == 'TEXTAREA' || edit.tagName == 'INPUT') {
						var sel_start = edit.selectionStart,
							sel_end = edit.selectionEnd;
		...
		moveRangeStart: function(container, range, amount) {
			...
			var result = (amount < 0) ? this.findIndexBehind(container, range.startContainer,
				range.startOffset, -amount, true) : this.findIndexAhead(container, range.startContainer,
			...
			if (result.amount < amount) {
				...
				if (node != container && canScanUp) {
					...
					var new_result = this.findIndexAhead(container,
						node.parentNode, node_index + 1, amount - result.amount, true);
	...
	module: {
		...
		onLanguageChange: function(evt) {
			...
			if (this.language != language) {
				...
				if (this.showLangInterface)
					this.launchLanguageInterface();
				this.launchKeyboard();
				this.launchConverter();

				if (tinyMCE && tinyMCE.activeEditor) {
					tinyMCE.activeEditor.fire('langchange', {
						lang: tag
					});
				}
			}
